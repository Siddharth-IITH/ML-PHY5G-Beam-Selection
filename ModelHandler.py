...
from tensorflow.python.keras.layers import BatchNormalization, LeakyReLU, Conv2D, add,\
...
class ModelHandler:
    ...
    def createArchitecture(self,model_type,num_classes,input_shape,chain):
        ...
        if(model_type == 'inception_single'):
            ...
            layer = Dense(16, use_bias=True,activation='relu')(input_coord)
            layer = Dense(16, use_bias=True,activation='relu')(layer)
            layer = Dense(16, use_bias=True,activation='relu')(layer)
            layer = Dense(16, use_bias=True,activation='relu')(layer)
            layer = Dense(64, use_bias=True,activation='relu')(layer)
            layer = Dense(64, use_bias=True,activation='relu')(layer)
            #changed from here - siddharth s 1.0
            layer = Dense(128, use_bias=True,activation='relu')(layer)
            layer = Dense(128, use_bias=True,activation='relu')(layer)
            layer = Dense(256, use_bias=True,activation='relu')(layer)
            layer = Dense(256, use_bias=True,activation='relu')(layer)
            #change end s 1.0
